    ...
    methods: {
      init (orgNo) {
        ...
        if (!this.dataForm.orgNo) {
          ...
          // 修改
          this.$get(`/sys/dept/info/${this.dataForm.orgNo}`).then((data) => {
            if (data && data.code === 0) {
                this.dataForm = data.org
                this.deptListTreeSetCurrentNode('mod')
            }
          })
        }
      })
      ...
      // 机构树设置当前选中节点
      deptListTreeSetCurrentNode (type) {
        if (type === 'mod') {
        ...
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            var data= this.dataForm
             this.$post(`/sys/dept/${!this.dataForm.orgNo ? 'save' : 'update'}`,data).then((data) => {
              if (data && data.code === 0) {
                this.$message({
                  message: '操作成功',
                  type: 'success',
