<template>
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()"
      ...
      <el-form-item v-if="dataForm.type != 0" label="组件路径" prop="url">
        <el-input v-model="dataForm.url" placeholder="组件路径"></el-input>
      </el-form-item>
  ...
  export default {
    ...
    methods: {
      init (id) {
        ...
        this.$get(`/sys/menu/select`).then((data) => {
          ...
          if (!this.dataForm.id) {
            ...
            // 修改
            this.$get(`/sys/menu/info/${this.dataForm.id}`).then((data) => {
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            this.$post(`/sys/menu/${!this.dataForm.id ? 'save' : 'update'}`,params).then((data) => {
              if (data && data.code === 0) {
                this.$message({
                  message: '操作成功',
                  type: 'success',
