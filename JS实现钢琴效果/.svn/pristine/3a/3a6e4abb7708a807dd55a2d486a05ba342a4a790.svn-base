...
import AddOrUpdate from './table1-add-or-update'
  export default {
    name: 'table',
    components: {},
    data() {
    ...
    methods: {
      ...
      getDataList(){
        ...
        this.$get('/project/photovoltaic/queryAll', params).then((response) => {
      ...
      deleteHandle(id){
        ...
        this.$post('/project/photovoltaic/delete', ids).then((data) => {
          if (data && data.code === 0) {
              this.$message({
                message: '操作成功',
                type: 'success',
      ...
      // 导出excel
      exportHandle (id) {
        window.location.href = 'http://localhost:8080/gs/project/photovoltaic/exportExcel?&token='+this.$cookie.get('token')
      },
      // 每页数
