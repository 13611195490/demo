<template>
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()"label-width="80px">
      ...
      <el-form-item label="场站当前状态" prop="stationNowState" style="display: inline-block;width:47%">
  ...
  export default {
    ...
    methods: {
      init (id) {
        ...
        if(this.dataForm.id){
          this.$get(`/project/centralized/info/${this.dataForm.id}`).then((data) => {
            if (data && data.code === 0) {
                this.dataForm.stationName = data.centralized.stationName
                this.dataForm.stationAddress = data.centralized.stationAddress
                this.dataForm.itemCompany = data.centralized.itemCompany
                this.dataForm.affiliatedGroup = data.centralized.affiliatedGroup
                this.dataForm.stationType = data.centralized.stationType
                this.dataForm.installedCapacity = data.centralized.installedCapacity
                this.dataForm.stationNowState = data.centralized.stationNowState
                this.dataForm.approvalTime = data.centralized.approvalTime
                this.dataForm.isImplement = data.centralized.isImplement.toString()
                this.dataForm.onGridTime = data.centralized.onGridTime
                this.dataForm.longitude = data.centralized.longitude
                this.dataForm.latitude = data.centralized.latitude
            }
          }).then(()=>{
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            this.$post(`/project/centralized/${!this.dataForm.id ? 'save' : 'update'}`,params).then((data) => {
              if (data && data.code === 0) {
                this.$message({
                  message: '操作成功',
                  type: 'success',
