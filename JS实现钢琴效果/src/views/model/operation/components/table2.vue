...
import AddOrUpdate from './table2-add-or-update'
  export default {
    name: 'table',
    components: {},
    data() {
    ...
    methods: {
      ...
      deleteHandle(id){
        ...
        this.$post('/project/helpThePoor/delete', ids).then((data) => {
          if (data && data.code === 0) {
              this.$message({
                message: '操作成功',
                type: 'success',
      ...
      // 导出excel
      exportHandle (id) {
        window.location.href = 'http://localhost:8080/gs/project/helpThePoor/exportExcel?&token='+this.$cookie.get('token')
      },
      // 每页数
